...
public class Watermark extends Module
{
    ...
    public void onRenderGameOverlay(RenderGameOverlayEvent event)
    {
        ...
        if(Position.getEnumValue().equalsIgnoreCase("Right Top")) {
            FontUtil.drawStringWithShadow("ChipsHack b9", screen.getScaledWidth_double() - FontUtil.getStringWidth("ChipsHack-b9") - 4, 2.0D , new Color(255, 255, 255, 232).getRGB() ); }

        if(Position.getEnumValue().equalsIgnoreCase("Right Bottom")) {
            FontUtil.drawStringWithShadow("ChipsHack b9", screen.getScaledWidth_double() - FontUtil.getStringWidth("ChipsHack-b9") - 4, screen.getScaledHeight_double() - FontUtil.getFontHeight() + 1 , new Color(255, 255, 255, 232).getRGB() ); }

        if(Position.getEnumValue().equalsIgnoreCase("Left Top")) {
            FontUtil.drawStringWithShadow("ChipsHack b9", 2.0D, 2.0D , new Color(255, 255, 255, 232).getRGB() ); }

        if(Position.getEnumValue().equalsIgnoreCase("Left Bottom")) {
            FontUtil.drawStringWithShadow("ChipsHack b9", 2.0D,  screen.getScaledHeight_double() - FontUtil.getFontHeight() - 2, new Color(255, 255, 255, 232).getRGB() ); }
    }
